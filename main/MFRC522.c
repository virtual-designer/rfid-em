...
static Uid uid;
static enum StatusCode state;

static gpio_num_t PIN_NUM_MISO;
static gpio_num_t PIN_NUM_MOSI;
static gpio_num_t PIN_NUM_SCLK;
static gpio_num_t PIN_NUM_CS;
static gpio_num_t PIN_NUM_RST;

void
PCD_SetPins (gpio_num_t miso, gpio_num_t mosi, gpio_num_t sclk, gpio_num_t cs,
             gpio_num_t rst)
{
    PIN_NUM_MISO = miso;
    PIN_NUM_MOSI = mosi;
 ...
 */
void
PCD_Version (spi_device_handle_t spi)
{

    uint8_t ver = PCD_ReadRegister (spi, VersionReg);

    printf ("Version:%x\r\n", ver);
    if (ver == 0x92 || ver == 0x82)
    {
 ...
 */
void
PCD_WriteRegister (spi_device_handle_t spi, uint8_t Register, uint8_t value)
{
...
void
PCD_WriteRegisterMany (spi_device_handle_t spi, uint8_t Register, uint8_t count,
                       uint8_t *values)
{
...
uint8_t
PCD_ReadRegister (spi_device_handle_t spi, uint8_t Register)
{
...
void
PCD_ReadRegisterMany (
    spi_device_handle_t spi,
    uint8_t
        Register, ///< The register to read from. One of the PCD_Register enums.
...
)
{
    ...
    esp_err_t ret;
    uint8_t reg = 0x80 | Register; // MSB == 1 is for reading. LSB is not used
                                   // in address. Datasheet section 8.1.2.3.
    spi_transaction_t t;
    memset (&t, 0, sizeof (t)); // Zero out the transaction
    t.length = 128;             // 8
    t.rxlength = 8 * count;
...
void
PCD_SetRegisterBitMask (spi_device_handle_t spi, uint8_t reg, uint8_t mask)
{
    uint8_t tmp;
    tmp = PCD_ReadRegister (spi, reg);
 ...
 */
uint8_t
PICC_HaltA (spi_device_handle_t spi)
{
    uint8_t result;
    uint8_t buffer[4];
    ...
    result = PCD_TransceiveData (spi, buffer, sizeof (buffer), NULL, 0, NULL, 0,
                                 false);
 ...
 */
void
PCD_StopCrypto1 (spi_device_handle_t spi)
{
...
void
PCD_Init (spi_device_handle_t spi)
{
    ...
    PCD_AntennaOn (spi); // Enable the antenna driver pins TX1 and TX2 (they
                         // were disabled by the reset)
    printf ("Initialization successful.\n");
}

void
PCD_AntennaOn (spi_device_handle_t spi)
{
...
bool
PICC_IsNewCardPresent (spi_device_handle_t spi)
{
...
void
PICC_GetTypeName (PICC_Type piccType)
{
    switch (piccType)
    {
 ...
 */
PICC_Type
PICC_GetType (uint8_t sak ///< The SAK byte returned from PICC_Select().
)
{
 ...
 */
uint8_t
PICC_RequestA (spi_device_handle_t spi, uint8_t *bufferATQA,
               uint8_t *bufferSize)
{
 ...
 */
uint8_t
PICC_REQA_or_WUPA (spi_device_handle_t spi, uint8_t command,
                   uint8_t *bufferATQA, uint8_t *bufferSize)
{
    ...
    PCD_ClearRegisterBitMask (spi, CollReg,
    ...
    status = PCD_TransceiveData (spi, &command, 1, bufferATQA, bufferSize,
                                 &validBits, 0, false);
    if (status != STATUS_OK)
    {
        return status;
...
PCD_TransceiveData (
    ...
    uint8_t *validBits, ///< In/Out: The number of valid bits in the last
                        ///< uint8_t. 0 for 8 valid bits. Default nullptr.
...
)
{
    ...
    returnval = PCD_CommunicateWithPICC (spi, PCD_Transceive, waitIRq, sendData,
...
PCD_CommunicateWithPICC (
    ...
    uint8_t *backData, ///< nullptr or pointer to buffer if data should be read
                       ///< back after executing the command.
    uint8_t *backLen,  ///< In: Max number of bytess to write to *backData. Out:
                       ///< The number of uint8_ts returned.
...
)
{
    ...
    PCD_WriteRegister (spi, ComIrqReg,
                       0x7F); // Clear all seven interrupt request bits
    PCD_WriteRegister (spi, FIFOLevelReg,
    ...
    //   PCD_WriteRegisterMany(spi,FIFODataReg,sendLen,sendData);  // Write
    //   sendData to the FIFO
    ...
    // the do-while-loop takes 17.86Î¼s.
    ...
    {
        n = PCD_ReadRegister (
            spi, ComIrqReg); // ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq
                             // IdleIRq HiAlertIRq LoAlertIRq ErrIRq TimerIRq

        if (n & waitIRq)
        { // 0x30             // One of the interrupts that signal success has
          // been set.
    ...
    uint8_t errorRegValue = PCD_ReadRegister (
        spi, ErrorReg); // ErrorReg[7..0] bits are: WrErr TempErr reserved
    ...
    if (backData && backLen)
    {
        ...
        *backLen = h; // Number of bytes returned
        ...
        //     PCD_ReadRegisterMany(spi,FIFODataReg, h, backData, rxAlign); //
        ...
        _validBits = PCD_ReadRegister (spi, ControlReg)
                     & 0x07; // RxLastBits[2:0] indicates the number of valid
                             // bits in the last received uint8_t. If this value
                             // is 000b, the whole uint8_t is valid.
        if (validBits)
        {
    ...
    //        uint8_t must be received. if (*backLen < 2 || _validBits != 0) {
    //            return STATUS_CRC_WRONG;
    //        }
    //        // Verify CRC_A - do our own calculation and store the control in
    //        controlBuffer. uint8_t controlBuffer[2]; uint8_t status =
    //        PCD_CalculateCRC(spi,&backData[0], *backLen - 2,
    //        &controlBuffer[0]); if (status != STATUS_OK) {
    //            return status;
    //        }
...
bool
PICC_ReadCardSerial (spi_device_handle_t spi)
{
    ...
    while (!uidComplete)
    {
        ...
        switch (cascadeLevel)
        {
            case 1:
                ...
                uidIndex = 0;
                useCascadeTag
                    = validBits && uid->size > 4; // When we know that the UID
            ...
            case 2:
                ...
                uidIndex = 3;
                useCascadeTag
                    = validBits && uid->size > 7; // When we know that the UID
        ...
        if (bytesToCopy)
        {
            ...
            uint8_t maxBytes = useCascadeTag
                                   ? 3
                                   : 4; // Max 4 bytes in each Cascade Level.
        ...
        while (!selectDone)
        {
            ...
            result
                = PCD_TransceiveData (spi, buffer, bufferUsed, responseBuffer,
                                      &responseLength, &txLastBits, rxAlign, 0);

            if (result == STATUS_COLLISION)
            { // More than one PICC in the field => collision.
                uint8_t valueOfCollReg = PCD_ReadRegister (
                    spi, CollReg); // CollReg[7..0] bits are: ValuesAfterColl
 ...
 */
bool
PCD_CalculateCRC (spi_device_handle_t spi, uint8_t *data, uint8_t length,
                  uint8_t *result)
{
    PCD_WriteRegister (spi, CommandReg, PCD_Idle); // Stop any active command.
    PCD_WriteRegister (spi, DivIrqReg,
                       0x04); // Clear the CRCIRq interrupt request bit
    PCD_WriteRegister (spi, FIFOLevelReg,
                       0x80); // FlushBuffer = 1, FIFO initialization
    PCD_WriteRegisterMany (spi, FIFODataReg, length,
...
void
PICC_DumpToSerial (spi_device_handle_t spi,
                   Uid *uid ///< Pointer to Uid struct returned from a
                            ///< successful PICC_Select().
)
{
    // UID
    MIFARE_Key key;
    ...
    switch (piccType)
    {
        ...
        case PICC_TYPE_MIFARE_PLUS:
        case PICC_TYPE_TNP3XXX:
            printf ("Dumping memory contents not implemented for that PICC "
                    "type.\n");
            break;
...
PICC_DumpMifareClassicToSerial (
    spi_device_handle_t spi, Uid *uid, ///< Pointer to Uid struct returned from
...
)
{
    ...
    for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--)
    {
        ...
        if (isSectorTrailer)
        {
            status = PCD_Authenticate (spi, PICC_CMD_MF_AUTH_KEY_A, firstBlock,
                                       key, uid);
            if (status != STATUS_OK)
            {
...
)
{
    ...
    status
        = PCD_CommunicateWithPICC (spi, PCD_MFAuthent, waitIRq, &sendData[0],
...
void
PICC_DumpDetailsToSerial (Uid *uid)
{
    printf ("Card UID:");
...
MIFARE_Read (
    ...
    uint8_t *bufferSize ///< Buffer size, at least 18 bytes. Also number of
...
)
{
    ...
    return PCD_TransceiveData (spi, buffer, 4, buffer, bufferSize, NULL, 0,
                               true);
} // End MIFARE_Read()
...
)
{
    ...
    uint8_t validBits = 0;
    result = PCD_CommunicateWithPICC (spi, PCD_Transceive, waitIRq, cmdBuffer,
 ...
 */
void
PICC_DumpMifareUltralightToSerial (spi_device_handle_t spi)
{
    uint8_t status;
    uint8_t byteCount;
    uint8_t buffer[18];
 ...
 */
void
GetStatusCodeName (uint8_t code ///< One of the StatusCode enums.
)
{
    switch (code)
    {
