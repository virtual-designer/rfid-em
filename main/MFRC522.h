...
enum PCD_Register
{
    ...
    CollReg
    = 0x0E
      << 1, // bit position of the first bit-collision detected on
            // the RF interface
    ...
    = 0x19 << 1, // defines demodulator settings
                 // 						  0x1A			// reserved for
                 // future use 						  0x1B			// reserved for future use
    MfTxReg
    = 0x1C << 1, // controls some MIFARE communication transmit parameters
    MfRxReg = 0x1D << 1, // controls some MIFARE communication receive
                         // parameters 						  0x1E			// reserved for future use
    SerialSpeedReg
    ...
    ModWidthReg
    = 0x24
      << 1, // controls the ModWidth setting?
    ...
    TestADCReg = 0x3B << 1     // shows the value of ADC I and Q channels
                           // 						  0x3C			// reserved
                           // for production tests 						  0x3D			// reserved for
                           // production tests 						  0x3E			// reserved for
                           // production tests 						  0x3F			// reserved for
                           // production tests
};
...
// (on the PCD). Described in 9.3.3.6 / table 98 of the datasheet at
...
enum PICC_Command
{
    ...
    = 0xE0, // Request command for Answer To Reset.
            ...
            // Use PCD_MFAuthent to authenticate access to a sector, then use
            // these commands to read/write/modify the blocks on the sector. The
...
const Uid *PICC_GetUid (void);
enum StatusCode PICC_GetState (void);
void PCD_SetPins (gpio_num_t miso, gpio_num_t mosi, gpio_num_t sclk,
                  gpio_num_t cs, gpio_num_t rst);
// Codes for writing to registers.
void PCD_WriteRegister (spi_device_handle_t spi, uint8_t Register,
                        uint8_t value);
void PCD_WriteRegisterMany (spi_device_handle_t spi, uint8_t reg, uint8_t count,
                            uint8_t *values);
// Codes for reading from registers.
uint8_t PCD_ReadRegister (spi_device_handle_t spi, uint8_t Register);
void PCD_ReadRegisterMany (spi_device_handle_t spi, uint8_t reg, uint8_t count,
                           uint8_t *values, uint8_t rxAlign);
// Codes for setting / clearing particular bits.
void PCD_ClearRegisterBitMask (spi_device_handle_t spi, uint8_t reg,
                               uint8_t mask);
void PCD_SetRegisterBitMask (spi_device_handle_t spi, uint8_t reg,
                             uint8_t mask);
...
uint8_t PICC_RequestA (spi_device_handle_t spi, uint8_t *bufferATQA,
                       uint8_t *bufferSize);
uint8_t PICC_REQA_or_WUPA (spi_device_handle_t spi, uint8_t command,
                           uint8_t *bufferATQA, uint8_t *bufferSize);
uint8_t PCD_TransceiveData (spi_device_handle_t spi, uint8_t *sendData,
                            uint8_t sendLen, uint8_t *backData,
                            uint8_t *backLen, uint8_t *validBits,
                            uint8_t rxAlign, bool checkCRC);
uint8_t PCD_CommunicateWithPICC (spi_device_handle_t spi, uint8_t command,
...
bool PCD_CalculateCRC (spi_device_handle_t spi, uint8_t *data, uint8_t length,
                       uint8_t *result);
...
void PICC_DumpToSerial (spi_device_handle_t spi,
